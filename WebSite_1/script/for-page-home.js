    ...
    if (el_email.length !== 0) { // Проверка поля Email
        if (el_email.length <= 50) {
            if ((el_email.length >= 5) && (el_email.indexOf("@") !== -1) && (el_email.indexOf(".") !== -1)) {
                ...
                document.querySelector(".ph-subscribe__form-btn").classList.add("done"); // ВВЕСТИ НАЗВАНИЕ СЕЛЕКТОРА КНОПКИ!!!
                setTimeout(() => { // Таймер для анимации кнопки отправки
                    document.querySelector(".ph-subscribe__form-btn").classList.remove("done"); // ВВЕСТИ НАЗВАНИЕ СЕЛЕКТОРА КНОПКИ!!!
                }, 5000);
            }
            else {
                    ...
                    if (el_sms.length !== 0) { // Проверка поля сообщения
                        if (el_sms.length >= 10) {
                            if (el_sms.length <= 100) {
                                console.log("Ваше сообщение отправлено!");
                                document.querySelector(".ph-contact__form-btn").classList.add("done"); // ВВЕСТИ НАЗВАНИЕ СЕЛЕКТОРА КНОПКИ!!!
                                setTimeout(() => { // Таймер для анимации кнопки отправки
                                    document.querySelector(".ph-contact__form-btn").classList.remove("done"); // ВВЕСТИ НАЗВАНИЕ СЕЛЕКТОРА КНОПКИ!!!
                                }, 5000);
                            }
                            else {
